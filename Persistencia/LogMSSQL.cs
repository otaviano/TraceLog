...
{
	internal class LogMSSQL : Abstracao.ILogDAO
	{
		...
		private Dominio.Log SalvarLog(Dominio.Log log)
		{
			...
			try
			{
				sql = "INSERT INTO TRACELOG.LOG_BASE (LOG_LOGIN, LOG_DATA)" +
							"VALUES(@LOG_LOGIN, GETDATE());" +
							"SELECT @@IDENTITY;";

				IDataParameter[] parametros = new IDataParameter[]
		    {
		      new SqlParameter("@LOG_LOGIN", log.Login)
		    };
		...
		public int SalvarErro(Dominio.LogErro erro, bool transacional = true)
		{
			...
			try
			{
				...
				sql = "INSERT INTO [TRACELOG].[LOG_ERRO](LOG_IDF, LOE_NOME_METODO, LOE_NOME_ENTIDADE, LOE_NOME_ERRO, LOE_DESCRICAO_ERRO, LOG_IDF_PAI)" +
							"VALUES (@LOG_IDF, @LOE_NOME_METODO, @LOE_NOME_ENTIDADE, @LOE_NOME_ERRO, @LOE_DESC_ERRO, @LOG_IDF_PAI)";

				IDataParameter[] parametros = new IDataParameter[]
		    {
		      new SqlParameter("@LOG_IDF", erro.Codigo),
		      new SqlParameter("@LOE_NOME_METODO", erro.NomeMetodo),
		      new SqlParameter("@LOE_NOME_ENTIDADE", erro.NomeEntidade),
		      new SqlParameter("@LOE_NOME_ERRO", erro.NomeErro),
		      new SqlParameter("@LOE_DESC_ERRO", erro.DescricaoErro),
		      new SqlParameter("@LOG_IDF_PAI", (erro.ErroPai != null) ? this.SalvarErro(erro.ErroPai, false) : (object) DBNull.Value)
		    };
		...
		public void SalvarTrace(Dominio.LogTrace trace, bool transacional = true)
		{
			...
			try
			{
				...
				IDataParameter[] parametros = new IDataParameter[]
		    {
		      new SqlParameter("@LOG_IDF", trace.Codigo),
		      new SqlParameter("@LOC_OBSERVACAO", trace.Observacao),
		...
		public void SalvarTransacao(Dominio.LogTransacao transacao, bool transacional = true)
		{
			...
			try
			{
				...
				sql = @"INSERT INTO [TRACELOG].[LOG_TRANSACAO] (LOG_IDF, LON_VERSAO_ANTERIOR, LON_OBSERVACAO, LON_NOME_ENTIDADE, LON_DATA_VALIDADE, LON_IDF_ENTIDADE, TIO_IDF)
						VALUES(@LOG_IDF, @LON_VERSAO_ANTERIOR, @LON_OBSERVACAO, @LON_NOME_ENTIDADE, @LON_DATA_VALIDADE, @LON_IDF_ENTIDADE,@TIO_IDF)";

			IDataParameter[] parametros = new IDataParameter[]
		    {
				new SqlParameter("@LOG_IDF", transacao.Codigo),
				new SqlParameter("@LON_VERSAO_ANTERIOR", (transacao.XmlVersao != null)? transacao.XmlVersao.InnerXml : (object) DBNull.Value),
				new SqlParameter("@LON_OBSERVACAO", (!string.IsNullOrEmpty(transacao.Observacao)) ? transacao.Observacao : (object) DBNull.Value),
				new SqlParameter("@LON_NOME_ENTIDADE", (!string.IsNullOrEmpty(transacao.NomeEntidade) ? transacao.NomeEntidade : (object) DBNull.Value)),
				new SqlParameter("@LON_DATA_VALIDADE", (transacao.DataValidade > DateTime.MinValue) ? transacao.DataValidade : (object) DBNull.Value),
				new SqlParameter("@LON_IDF_ENTIDADE", transacao.CodigoEntidade),
				new SqlParameter("@TIO_IDF", Convert.ToByte(transacao.Tipo))
		    };
		...
		public List<Dominio.LogErro> PesquisarErro(Dominio.LogErro erro)
		{
			...
			try
			{
				sql = @"SELECT L.LOG_IDF, L.LOG_LOGIN, L.LOG_DATA, LOE_NOME_METODO, LOE_NOME_ENTIDADE, LOE_NOME_ERRO, LOE_DESCRICAO_ERRO, LOG_IDF_PAI
						FROM TRACELOG.LOG_BASE L
						INNER JOIN TRACELOG.LOG_ERRO E ON (E.LOG_IDF = L.LOG_IDF)
				...
				IDataParameter[] parametros = new IDataParameter[]
				{
					new SqlParameter("@LOG_IDF", (erro.Codigo > 0) ? erro.Codigo : (object) DBNull.Value),
					new SqlParameter("@LOE_NOME_ENTIDADE", (!string.IsNullOrEmpty(erro.NomeEntidade)) ? erro.NomeEntidade : (object) DBNull.Value),
					new SqlParameter("@LOG_DATA", (erro.Data > DateTime.MinValue) ? erro.Data : (object) DBNull.Value),
					new SqlParameter("@LOE_NOME_ERRO", (!string.IsNullOrEmpty(erro.NomeErro) ? erro.NomeErro : (object) DBNull.Value)),
					new SqlParameter("LOG_IDF_PAI", (erro.ErroPai != null) ? erro.ErroPai.Codigo : (object) DBNull.Value)
				};
		...
		public List<Dominio.LogTrace> PesquisarTrace(Dominio.LogTrace trace)
		{
			...
			try
			{
				sql = @"SELECT L.LOG_IDF,L.LOG_LOGIN,L.LOG_DATA, LOC_OBSERVACAO
								FROM TRACELOG.LOG_BASE L
								INNER JOIN TRACELOG.LOG_TRACE T ON (T.LOG_IDF = L.LOG_IDF)";
		...
		public List<Dominio.LogTransacao> PesquisarTransacao(Dominio.LogTransacao transacao)
		{
			...
			try
			{
				sql = @"SELECT  L.LOG_IDF, L.LOG_LOGIN, L.LOG_DATA, T.LON_NOME_ENTIDADE, T.LON_VERSAO_ANTERIOR, 
						...
						FROM TRACELOG.LOG_BASE L
						INNER JOIN TRACELOG.LOG_TRANSACAO T ON (T.LOG_IDF = L.LOG_IDF)
						INNER JOIN TRACELOG.TIPO_OPERACAO TI ON (TI.TIO_IDF = T.TIO_IDF)
						WHERE	(@LOG_IDF IS NULL OR L.LOG_IDF = @LOG_IDF)
				...
				IDataParameter[] parametros = new IDataParameter[]
		    {
		      new SqlParameter("@LOG_IDF", (transacao.Codigo > 0) ? transacao.Codigo : (object) DBNull.Value),
			  new SqlParameter("@LON_IDF_ENTIDADE", (transacao.CodigoEntidade > 0) ? transacao.CodigoEntidade : (object) DBNull.Value),
			  new SqlParameter("@LON_NOME_ENTIDADE", (!string.IsNullOrEmpty(transacao.NomeEntidade)) ? transacao.NomeEntidade : (object) DBNull.Value)
		    };
